...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _changeVideoFilter(int index) {
    setState(() => _activeVideoFilter = index);
    _fetchVideos(refresh: true);
  }
            ...
            child: Center(
              child: ShaderMask(
                ...
                child: const Text('Soul',
                  style: TextStyle(fontSize: 32, fontWeight: FontWeight.w300, fontStyle: FontStyle.italic, color: Colors.white, letterSpacing: 2)),
              ),
            ),
          ),
          Expanded(
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  Text(cat['name'] ?? '',
                    style: TextStyle(fontSize: 15, color: isActive ? Colors.white : Colors.white60,
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
            ...
            children: List.generate(_videoFilters.length, (index) {
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  child: Text(_videoFilters[index]['label']!,
                    style: TextStyle(fontSize: 14, color: isActive ? Colors.white : Colors.white54,
                      fontWeight: isActive ? FontWeight.w600 : FontWeight.normal)),
                ),
              );
            }),
          ...
          GestureDetector(
            onTap: () => setState(() => _gridMode = _gridMode == 1 ? 2 : 1),
            child: Icon(_gridMode == 1 ? Icons.view_list : Icons.grid_view, color: Colors.white70, size: 18),
          ),
        ],
      ),
    );
  ...
  Widget _buildVideoGrid() {
    if (_videos.isEmpty && _isLoading) {
      return const SliverFillRemaining(child: Center(child: CircularProgressIndicator(color: AppTheme.primaryColor)));
    }
      ...
      return SliverFillRemaining(
        child: Center(
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const SizedBox(height: 16),
            ElevatedButton(onPressed: () => _fetchVideos(refresh: true), child: const Text('重试')),
          ]),
        ),
      );
    }
    if (_videos.isEmpty) {
      return const SliverFillRemaining(child: Center(child: Text('暂无视频', style: TextStyle(color: Colors.white38))));
    }
    return SliverPadding(
      padding: const EdgeInsets.symmetric(horizontal: 6),
      sliver: SliverGrid(
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
          crossAxisCount: _gridMode == 1 ? 1 : 2,
          childAspectRatio: _gridMode == 1 ? 1.6 : 0.68,
          crossAxisSpacing: 8, mainAxisSpacing: 8),
        delegate: SliverChildBuilderDelegate(
          (context, index) => _buildVideoCard(_videos[index]),
  ...
  Widget _buildVideoCard(Video video) {
    ...
    return GestureDetector(
      onTap: () => Navigator.pushNamed(context, '/video', arguments: {'videoId': video.id}),
      child: Container(
              ...
              child: Stack(fit: StackFit.expand, children: [
                coverUrl.isNotEmpty
                    ? CachedNetworkImage(imageUrl: coverUrl, fit: BoxFit.cover,
                        ...
                        errorWidget: (_, __, ___) => Container(color: const Color(0xFF252525), child: const Icon(Icons.image, color: Colors.white24)))
