    ...
    <div class="form-content">
      ...
      <div class="upload-section">
        <div class="section-header">
          <span class="section-title">添加视频</span>
          <span class="section-tip">最大300M以内 请选择30S以上视频</span>
        </div>
        <div class="upload-grid">
          <!-- 封面上传 -->
          <div class="upload-item" :class="{ 'has-image': coverPreview }" @click="triggerCoverUpload">
            <img v-if="coverPreview" :src="coverPreview" class="preview-img" />
            <template v-else>
              <span class="plus">+</span>
              <span class="text">上传封面</span>
            </template>
            <span v-if="coverPreview" class="remove-btn" @click.stop="removeCover">×</span>
          </div>
          <!-- 视频上传 -->
          <div class="upload-item" :class="{ 'has-video': videoFile }" @click="triggerVideoUpload">
            <video v-if="videoPreview" :src="videoPreview" class="preview-video"></video>
            <template v-else>
              <span class="plus">+</span>
              <span class="text">上传视频</span>
            </template>
            <span v-if="videoFile" class="remove-btn" @click.stop="removeVideo">×</span>
          </div>
        </div>
        <input type="file" ref="coverInput" accept="image/*" @change="handleCoverSelect" style="display:none" />
      ...
      <div class="upload-section">
        ...
        <div class="upload-grid">
          ...
          <div class="upload-item add-btn" @click="triggerImageUpload" v-if="images.length < 9">
            <span class="plus">+</span>
            <span class="text">上传图片</span>
          </div>
        </div>
        <input type="file" ref="imageInput" accept="image/*" multiple @change="handleImageSelect" style="display:none" />
      </div>
    </div>
...
const canSubmit = computed(() => {
  return title.value.trim() && videoFile.value
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const videoRes = await api.post('/community/upload/video', videoFormData, {
    ...
    if (coverFile.value) {
      ...
      const coverRes = await api.post('/community/upload/image', coverFormData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    ...
    await api.post('/community/posts', {
      content: `${title.value}\n\n${content.value}`,
      images: coverUrl ? [coverUrl, ...uploadedUrls] : uploadedUrls,
      video_url: videoUrl,
