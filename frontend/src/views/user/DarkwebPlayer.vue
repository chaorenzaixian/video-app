...
const shareFullUrl = computed(() => `${window.location.origin}/user/darkweb/video/${video.value.id}?ref=${userInviteCode.value}`)
...
const fetchVideo = async () => {
  ...
  try {
    const res = await api.get(`/darkweb/videos/${videoId}`, { signal: abortSignal })
    ...
    await api.post(`/darkweb/videos/${videoId}/view`)
  } catch (error) {
...
const fetchRecommend = async () => {
  try {
    // 使用随机排序获取暗网推荐视频
    const res = await api.get('/darkweb/videos', { params: { page: 1, page_size: 12, sort: 'random' }, signal: abortSignal })
    const data = res.data || res
...
const toggleLike = async () => {
  try {
    const res = await api.post(`/darkweb/videos/${video.value.id}/like`)
    const data = res.data || res
...
const toggleFavorite = async () => {
  try {
    const res = await api.post(`/darkweb/videos/${video.value.id}/favorite`)
    const data = res.data || res
...
const goToVideo = (id) => {
  ...
  // 强制刷新页面以加载新视频
  router.push(`/user/darkweb/video/${id}`).then(() => {
    window.scrollTo(0, 0)
  })
}
...
.comments-content {
  ...
  @keyframes supreme-glow {
    0%, 100% {
      filter: drop-shadow(0 0 4px rgba(255, 68, 68, 0.6)) drop-shadow(0 0 8px rgba(255, 100, 100, 0.4));
      transform: scale(1);
    }
    50% {
      filter: drop-shadow(0 0 8px rgba(255, 68, 68, 1)) drop-shadow(0 0 16px rgba(255, 100, 100, 0.8));
      transform: scale(1.1);
    }
  }
