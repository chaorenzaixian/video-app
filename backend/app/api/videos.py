...
async def upload_custom_cover(
    ...
    if file.content_type not in ImageService.SUPPORTED_FORMATS:
        raise HTTPException(status_code=400, detail="只支持 JPG、PNG、WebP、GIF、BMP 格式")
    ...
    try:
        result = await ImageService.save_image(
            content=content,
            subdir=f"temp_covers/{current_user.id}",
            filename=f"custom_{uuid.uuid4()}",
            convert_webp=True
        )
        ...
        with open(cover_path, "wb") as f:
            f.write(content)
        return {"url": f"/uploads/temp_covers/{current_user.id}/{cover_filename}", "message": "上传成功", "optimized": False}
